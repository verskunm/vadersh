(function ( $ ) {
	...
	$(function () {
		...
		$( 'div.comment-weight-container' ).on( 'click', 'span > a', _.throttle( function( e ){
			...
			var qs_comment_id = $(this).data('comment-id');
			$('.vote-up[data-comment-id='+qs_comment_id+']').hide();
			$('.vote-down[data-comment-id='+qs_comment_id+']').hide();
			$('#comment-weight-value-'+qs_comment_id+' .current-vote').hide();
			$('#comment-weight-value-'+qs_comment_id+' .loading').show();
